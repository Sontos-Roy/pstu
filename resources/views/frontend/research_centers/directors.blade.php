        ...
        <div class="row">
            <div class="advisor-items col-3 text-light text-center">
                ...
                <div class="col-md-4 col-sm-6 single-item">
                    <a href="{{ route('front.user.profile', $item->user_id) }}">
                    </a>
                    <div class="item">
                        <a href="{{ route('front.user.profile', $item->user_id) }}">
                            <div class="thumb">
                        ...
                        <div class="info" style="height: 140px">
                            <a href="{{ route('front.user.profile', $item->user_id) }}">
                            </a><a href="{{ route('front.user.profile', $item->user_id) }}">
                            <span>{{ $item->name }}</span>
                            </a>
                            <h4>
                                <a href="{{ route('front.user.profile', $item->user_id) }}" target="_blank"> {{ $item->user->name }}
                                </a>
                            </h4>
                            <span>
                            </span>
                        ...
                        <div class="text-center">
                            <a class="btn circle btn-dark border btn-sm" href="{{ route('front.user.profile', $item->user_id) }}" style="margin:10px;">Read More...<i class="fas fa-check-circle fa-2x fa-pull-right"></i></a>
                        </div>
                    </div>
                </div>
                @endforeach
